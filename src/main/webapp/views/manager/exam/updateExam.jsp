                ...
                <div class="panel-body">
                    <form role="form">
                      <input type="hidden" id="id" value="${param.id}"/>
                        ...
                        <button type="button" class="btn btn-success" id="update_exam_btn"><i class="glyphicon glyphicon-plus"></i> 提交</button>
                    </form>
                </div>
            </div>
...
<script>
    ...
    //初始化界面，即加载本条exam原本的信息
    function initPage() {
        var id=$("#id").val();
        $.ajax({
            url:"${APP_PATH}/exam/getExam",
            type:"get",
            data:{
                "id":id
            },
            ...
            success:function(result) {
                ...
                weekBind(exam.availableRoom.week.id);
                timeBind(exam.availableRoom.period.id);
                roomBind(exam.availableRoom.week.id,exam.availableRoom.period.id,exam.availableRoom.id);
                lessonBind(exam.lesson.id);
                var teachers=exam.teachers;
                var teacherIds=new Array();
                $.each(teachers,function (i,teacher) {
                    teacherIds[i]=teacher.id;
                });
                teacherBind(teacherIds);
                $("#room_select").attr("default_value",exam.availableRoom.id);
            }
    ...
    //改变teacher下拉列表的函数
    function teacherBind(teacherId) {
        var teacher_selector=$("#teacher_select");
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                teacher_selector.val(teacherId).select2();
            }
        });
    }
    ...
    $("#update_exam_btn").click(function () {
        ...
        if(teacherIds==null){
            layer.msg('请选择教师',{time:3000,icon:5,shift:6},function () {});
            return;
        }
        var tIdsData="";

        $.each(teacherIds,function (i,teacherId) {
            tIdsData+=teacherId;
            tIdsData+=",";
        });
        var jsonData={
        ...
        if(default_availableRoomId!=availableRoomId){
            ...
            $.ajax({
                url:"${APP_PATH}/exam/validateExam",
                type:"post",
                data:{
                    "availableRoomId":availableRoomId
                },
    ...
    //提交修改信息的函数
    function updateExam(jsonData){
        $.ajax({
            url:"${APP_PATH}/exam/updateExam",
            data:jsonData,
            type:"post",
            beforeSend:function () {
            ...
            success:function(result) {
                ...
                if(flag){
                    ...
                    window.location.href="${APP_PATH}/views/manager/exam/manageExam.jsp";
                }else{
