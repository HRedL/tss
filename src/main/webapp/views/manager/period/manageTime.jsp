        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><i class="glyphicon glyphicon-th"></i>考试时间管理</h3>
                </div>
                    ...
                    <div class="table-responsive">
                        <form id="user_form" style="margin-top: -10px">
                            <table class="table table-hover">
                                <thead>
                                <tr >
                                    <th width="30"><input type="checkbox" style="margin-top: -20px"/></th>
                                    <th>开始时间</th>
                                    <th>结束时间</th>
                                    <th width="160">操作</th>
                                </tr>
...
<script type="text/javascript">
    ...
    function pageQuery(pageNumber){
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                var periods=pageInfo.list;
                $.each(periods,function (i,period) {
                    tbodyInf+='<tr>';
                    tbodyInf+='<td><input type="checkbox" class="check_item" value="'+period.id+'"/></td>';
                    tbodyInf+='<td>'+period.starttime+'</td>';
                    tbodyInf+='<td>'+period.endtime+'</td>';
                    tbodyInf+='<td>';
                    tbodyInf+='<button type="button" idClass= "'+period.id+'" class="btn btn-warning btn-xs update_btn"><i class=" glyphicon glyphicon-pencil"></i>修改</button>';
                    tbodyInf+='<button type="button" idClass= "'+period.id+'" class="btn btn-danger btn-xs del_btn"><i class=" glyphicon glyphicon-remove"></i>删除</button>';
                    tbodyInf+='</td>';
                    tbodyInf+='</tr>';
                });
    ...
    $("#add_btn").click(function(){
        window.location.href="${APP_PATH}/views/manager/period/addTime.jsp";
    });

    $(document).on("click",".update_btn",function (){
        var id=$(this).attr("idClass");
        window.location.href="${APP_PATH}/views/manager/period/updateTime.jsp?id="+id;
    });
    ...
    $("#del_all_btn").click(function () {
        ...
        if(number == 0){
            layer.msg('请选择删除的时间',{time:3000,icon:1,shift:6});
        }else{
            layer.confirm("删除已选择的时间信息,是否继续",{icon:3,title:"提示"},function (cindex) {
                $.ajax({
                    url:"${APP_PATH}/period/deleteTimes",
                    type:"post",
                    traditional: true,
                    data:{
                        id:item
                    ...
                    success:function (result) {
                        if(result.success){
                            layer.close(cindex);
                            layer.msg('时间信息删除成功',{time:3000,icon:1,shift:6});
                            window.location.reload();
                        }else{
                            layer.msg('时间信息删除失败',{time:3000,icon:2,shift:6});
                        }
                    }
                });
            });
