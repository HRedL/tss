...
<script type="text/javascript">
    ...
    function pageQuery(pageNumber){
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                var weeks=pageInfo.list;
                $.each(weeks,function (i,week) {
                    tbodyInf+='<tr>';
                    tbodyInf+='<td><input type="checkbox" class="check_item" value="'+week.id+'"/></td>';
                    tbodyInf+='<td>'+week.term+'</td>';
                    tbodyInf+='<td>'+week.date+'</td>';
                    tbodyInf+='<td>';
                    tbodyInf+='<button type="button" idClass= "'+week.id+'" class="btn btn-warning btn-xs update_btn"><i class=" glyphicon glyphicon-pencil"></i>修改</button>';
                    tbodyInf+='<button type="button" idClass= "'+week.id+'" class="btn btn-danger btn-xs del_btn"><i class=" glyphicon glyphicon-remove"></i>删除</button>';
                    tbodyInf+='</td>';
                    tbodyInf+='</tr>';
                });
    ...
    $("#add_btn").click(function(){
        window.location.href="${APP_PATH}/views/manager/week/addWeek.jsp";
    });

    $(document).on("click",".update_btn",function (){
        var id=$(this).attr("idClass");
        window.location.href="${APP_PATH}/views/manager/week/updateWeek.jsp?id="+id;
    });
    ...
    $("#del_all_btn").click(function () {
        ...
        if(number == 0){
            layer.msg('请选择删除的日期',{time:3000,icon:1,shift:6});
        }else{
            layer.confirm("删除已选择的日期信息,是否继续",{icon:3,title:"提示"},function (cindex) {
                $.ajax({
                    url:"${APP_PATH}/week/deleteWeeks",
                    type:"post",
                    traditional: true,
                    data:{
                        id:item
                    ...
                    success:function (result) {
                        if(result.success){
                            layer.close(cindex);
                            layer.msg('日期信息删除成功',{time:3000,icon:1,shift:6});
                            window.location.reload();
                        }else{
                            layer.msg('日期信息删除失败',{time:3000,icon:2,shift:6});
                        }
                    }
                });
            });
