		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><i class="glyphicon glyphicon-th"></i>考试周管理</h3>
				</div>
				<div class="panel-body">
					...
					<button type="button" class="btn btn-primary" style="float:right;" id="add_week_btn"><i class="glyphicon glyphicon-plus"></i> 新增</button>
					<br>
					<hr style="clear:both;">
					<div class="table-responsive">
						<form id="user_form">
							<table class="table table-hover">
								...
								<!--数据信息-->
								<tbody id="userData">
								<c:forEach items="${pageInfo.list}" var="week">
									<tr>
								...
								<tr >
									<td colspan="6" align="center">
										...
										<ul id="pageInf" class="pagination">
											<li><a href="<c:url value="/week/list?pageNumber=1"/>">首页</a></li>
											<c:if test="${pageInfo.hasPreviousPage}">
												<li>
													<a href="<c:url value="/week/list?pageNumber=${pageInfo.pageNum-1}"/>" aria-label="Previous">
											...
											<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
												<c:if test="${pageNum==pageInfo.pageNum}">
													<li class="active"><a href="<c:url value="/week/list?pageNumber=${pageNum}"/>">${pageNum}</a></li>
												</c:if>
												<c:if test="${pageNum!=pageInfo.pageNum}">
													<li><a href="<c:url value="/week/list?pageNumber=${pageNum}"/>">${pageNum}</a></li>
												</c:if>
											</c:forEach>
											<c:if test="${pageInfo.hasNextPage}">
												<li>
													<a href="<c:url value="/week/list?pageNumber=${pageInfo.pageNum+1}"/>" aria-label="Next">
														<span aria-hidden="true">&raquo;</span>
													</a>
												</li>
											</c:if>
											<li><a href="<c:url value="/week/list?pageNumber=${pageInfo.pages}"/>">末页</a></li>
										</ul>
									</td>
								</tr>
								</tfoot>
...
<script type="text/javascript">
    ...
    $("#add_week_btn").click(function(){
        window.location.href="<c:url value="/week/toAddPage"/>";
    });
