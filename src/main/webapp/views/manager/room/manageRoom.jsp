        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><i class="glyphicon glyphicon-th"></i>教室管理</h3>
                </div>
...
<script type="text/javascript">
    ...
    function pageQuery(pageNumber){
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                var rooms=pageInfo.list;
                var pageSize=0;
                $.each(rooms,function (i,room) {
                    tbodyInf+='<tr>';
                    tbodyInf+='<td><input type="checkbox" class="check_item" value="'+room.id+'"/></td>';
                    tbodyInf+='<td>'+room.building.campus+':'+room.building.bnum+'</td>';
                    tbodyInf+='<td>'+room.rnum+'</td>';
                    tbodyInf+='<td>'+room.capacity+'</td>';
                    tbodyInf+='<td>'+room.ttotalnum+'</td>';
                    tbodyInf+='<td>';
                    tbodyInf+='<button type="button" idClass= "'+room.id+'" class="btn btn-warning btn-xs update_btn"><i class=" glyphicon glyphicon-pencil"></i>修改</button>';
                    tbodyInf+='<button type="button" idClass= "'+room.id+'" class="btn btn-danger btn-xs del_btn"><i class=" glyphicon glyphicon-remove"></i>删除</button>';
                    tbodyInf+='</td>';
                    tbodyInf+='</tr>';
                    pageSize++;
                    //中止循环
    ...
    $("#add_btn").click(function(){
        window.location.href="${APP_PATH}/views/manager/room/addRoom.jsp";
    });

    $(document).on("click",".update_btn",function (){
        var id=$(this).attr("idClass");
        window.location.href="${APP_PATH}/views/manager/room/updateRoom.jsp?id="+id;
    });
    ...
    $("#del_all_btn").click(function () {
        ...
        if(number == 0){
            layer.msg('请选择删除的教室',{time:3000,icon:1,shift:6});
        }else{
            layer.confirm("删除已选择的教室信息,是否继续",{icon:3,title:"提示"},function (cindex) {
                $.ajax({
                    url:"${APP_PATH}/room/deleteRooms",
                    type:"post",
                    traditional: true,
                    data:{
                        id:item
                    ...
                    success:function (result) {
                        if(result.success){
                            layer.close(cindex);
                            layer.msg('教室信息删除成功',{time:3000,icon:1,shift:6});
                            window.location.reload();
                        }else{
                            layer.msg('教室信息删除失败',{time:3000,icon:2,shift:6});
                        }
                    }
                });
            });
