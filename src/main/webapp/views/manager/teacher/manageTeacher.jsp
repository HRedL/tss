		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title"><i class="glyphicon glyphicon-th"></i>教师管理</h3>
				</div>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success:function(result) {
                ...
                $.each(teachers,function (i,teacher) {
                    tbodyInf+='<tr>';
                    tbodyInf+='<td><input type="checkbox" class="check_item" value="'+teacher.id+'"/></td>';
                    tbodyInf+='<td>'+teacher.tnum+'</td>';
                    tbodyInf+='<td>'+teacher.tname+'</td>';
                    if(teacher.sex==null){
                        tbodyInf+='<td>'+"未设置"+'</td>';
                    }
                    else {
                        tbodyInf += '<td>' + teacher.sex + '</td>';
                    }
                    if(teacher.academy==null){
                        tbodyInf+='<td>'+"未设置"+'</td>';
                    }
                    else {
                        tbodyInf += '<td>' + teacher.academy + '</td>';
                    }
                    if(teacher.dept==null){
                        tbodyInf+='<td>'+"未设置"+'</td>';
                    }
                    else {
                        tbodyInf += '<td>' + teacher.dept + '</td>';
                    }
                    tbodyInf += '<td>' + teacher.tinvtimes + '</td>';
                    if(teacher.ttel==null){
                        tbodyInf+='<td>'+"未设置"+'</td>';
                    }
                    else {
                        tbodyInf += '<td>' + teacher.ttel + '</td>';
                    }
                    tbodyInf+='<td>';
                    tbodyInf+='<button type="button" idClass= "'+teacher.id+'" class="btn btn-warning btn-xs update_btn"><i class=" glyphicon glyphicon-pencil"></i>修改</button>';
                    tbodyInf+='<button type="button" idClass= "'+teacher.id+'" class="btn btn-danger btn-xs del_btn"><i class=" glyphicon glyphicon-remove"></i>删除</button>';
                    tbodyInf+='</td>';
                    tbodyInf+='</tr>';
                });
    ...
    $("#add_btn").click(function(){
        window.location.href="${APP_PATH}/views/manager/teacher/addTeacher.jsp";
    });

    $(document).on("click",".update_btn",function (){
        var id=$(this).attr("idClass");
        window.location.href="${APP_PATH}/views/manager/teacher/updateTeacher.jsp?id="+id;
    });
    ...
    $("#del_all_btn").click(function () {
        ...
        if(number == 0){
            layer.msg('请选择删除的教师',{time:3000,icon:1,shift:6});
        }else{
            layer.confirm("删除已选择的教师信息,是否继续",{icon:3,title:"提示"},function (cindex) {
                $.ajax({
                    url:"${APP_PATH}/teacher/deleteTeachers",
                    type:"post",
                    traditional: true,
                    data:{
                        id:item
                    ...
                    success:function (result) {
                        if(result.success){
                            layer.close(cindex);
                            layer.msg('教师信息删除成功',{time:3000,icon:1,shift:6});
                            window.location.reload();
                        }else{
                            layer.msg('教师信息删除失败',{time:3000,icon:2,shift:6});
                        }
                    }
                });
            });
